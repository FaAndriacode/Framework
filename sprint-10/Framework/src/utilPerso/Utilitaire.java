...
import etu1763.framework.servlet.Front_Servlet;
import java.io.File;
...
public class Utilitaire {
        ...
        public static void whoClass(String pkg,Front_Servlet frontservlet) throws Exception{           
            List<Class> classe = getClasses(pkg);            
            setClass_annotation(classe,frontservlet);            
        }
        
        public static void setClass_annotation(List<Class> c,Front_Servlet frontservlet) throws Exception{           
            Method[] methods;
            
            Annotation annotM ,annotP ,annotS;
            
            Method annotMeth ,annotParams ,annotScope;
            ...
            for (Class classe : c) {
                ...
                if(classe.isAnnotationPresent(Scope.class)){
                   annotS = classe.getAnnotation(Scope.class);
                   annotScope = annotS.annotationType().getDeclaredMethod("valeur");
                   valueScope = annotScope.invoke(annotS).toString();
                   if(valueScope.equalsIgnoreCase("singleton")){
                       singleton.put(classe, null);
                   }
                }
            } 
            frontservlet.setMappingUrls(mappingUrls);
            frontservlet.setSingleton(singleton);
        }
